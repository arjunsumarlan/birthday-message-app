...
describe("User API", () => {
    ...
    test("PUT /user/:id should update user details and manage lastMessageSent correctly", async () => {
        ...
        const updatedData = { birthday: new Date(`${nextYear}-06-01`).toISOString() }; // Updating to next year's birthday

        const response = await request(app).put(`/user/${user._id}`).send(updatedData);
        expect(response.status).toBe(200);
        expect(new Date(response.body.birthday)).toEqual(new Date(updatedData.birthday));
